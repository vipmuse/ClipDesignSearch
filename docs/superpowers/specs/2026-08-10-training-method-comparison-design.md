...
**헤드명사 축은 한 개의 범용 통이 지배한다** (`data/pairs.jsonl` 실측, 2026-08-11):

```
전치사·보어가 헤드를 물품에서 밀어내는 레코드: 95,660건 / 20.2%
  'Front bumper for a vehicle' → vehicle · 'Container with cap' → cap
  'Food processor and parts thereof' → thereof
  대상 쌍 중 한쪽 이상이 이런 제목인 비율: 37.1%
배치(32)당 같은 헤드명사·다른 제목 쌍 3.096개의 출처:
  device                                          1.472쌍 (47.5%)
  device·assembly·apparatus·system·unit·interface  1.816쌍 (58.7%)
  box                                             약 0.02쌍
```

즉 그래디언트의 **절반가량은 범용 접미사만 공유하는 무관한 물품끼리를 갈라놓는 데
쓰인다** — 1차 설계가 반증된 바로 그 실패 모드(무관한 `Shoe`/`Bottle`을 민다)가 이번에는
헤드명사 축을 통해 다시 들어온 것이다. `Pizza box`/`Storage box`처럼 스펙이 근거로 든
목표 쌍은 실제로는 배치당 0.02쌍 수준이다.

따라서 **ablation의 Δ는 "범용 헤드명사를 공유하는 쌍을 밀어냈을 때의 효과"로만 읽어야
한다.** 스펙이 목표로 제시한 물품군(box, carrier, panel 계열)에 대한 증거가 아니다.
불용 헤드 목록이나 헤드 추출 규칙 변경은 설계 결정이라 이 브랜치에서 하지 않는다 (§7.4).

**가중치: 1.0** (2026-08-11 실측으로 확정). CLIP 항은 `logit_scale` 증폭을 받지만 hinge는
받지 않으므로 스케일 비를 추측하지 않고 실행 로그로 쟀다 — `--limit 3000 --max-steps 20
--log-every 1` 20스텝에서 가중 후 TIC이 평균 0.0446, 총 손실 평균 2.14의 **2.08%**였다.
(초안의 `tic_weight 0.2`는 선택 규칙이 바뀌기 전 값이라 더 이상 유효하지 않다. 선택 규칙
변경으로 대상 쌍이 165배 줄면서 `.mean()`의 분모가 작아져 손실 스케일 자체가 달라졌다.)
로그는 가중 전 값을 찍으므로 다른 가중치에서의 몫을 로그만으로 다시 계산할 수 있다.
...
## 7.4 tic 구현 후 남은 후속 항목

`feature/tic-loss` 최종 리뷰에서 확인된, 이번 브랜치에서 처리하지 않고 넘기는 것들.

- **헤드명사 축이 범용 통 하나에 쏠려 있다** (수치는 §3.2). 배치당 대상 쌍의 47.5%가
  헤드명사 `device` 하나에서, 58.7%가 범용 헤드 6종(`device`·`assembly`·`apparatus`·
  `system`·`unit`·`interface`)에서 나온다. 레코드의 20.2%는 전치사·보어 때문에 헤드가
  물품에서 밀려나 있다(`... for a vehicle` → `vehicle`, `... and parts thereof` →
  `thereof`). **`tic` arm의 Δ를 목표 물품군(box·carrier·panel 계열)에 대한 증거로 읽으면
  안 된다** — 그 쌍들은 배치당 0.02쌍 수준이라 Δ에 사실상 기여하지 않는다.
  처방 후보는 (a) 범용 헤드 불용 목록, (b) 전치사 앞까지만 헤드로 보는 추출 규칙,
  (c) 헤드명사 대신 로카르노 소분류로 물품군을 정의 — 셋 다 선택 규칙 자체를 바꾸는
  설계 결정이라 소유자 판단이 필요하고, 지금 넣으면 이 브랜치의 축이 둘로 늘어난다.
- **상한·바닥값·가중치는 전부 베이스 모델 코사인 분포 기준이다.** 학습이 진행되며
  텍스트 임베딩이 이동하면 대상/위반 비율이 달라진다. 로그의 위반/대상 카운트를 실제
  전체 학습(10 에폭)에서 관찰해야 하며, `tic_weight 1.0`의 근거인 2.08%도 20스텝
  창에서 잰 값이다.
- **`tic_ceiling` 스윕은 아직 안 돌렸다.** 스펙이 스윕 1순위로 지목한 손잡이지만, 위
  편중 문제가 먼저 정리되지 않으면 스윕 결과도 범용 헤드 쌍에 대한 곡선일 뿐이다.
